...
| M5 | 任务管理 + 自主性 | ✅ 已完成 | 100% |
| M6 | 自我进化能力 | ✅ 已完成 | 100% |
| M7 | CLI & Gateway | ✅ 已完成 | 100% |
| M8 | System Prompt 优化 | ✅ 已完成 | 100% |
| M9 | System Prompt 优化增强 | 🆕 待开始 | 0% |

---

## M8: System Prompt 优化 (🆕 待开始)
...
**验收标准**:
- [ ] `pc gateway restart` 正确停止并重新启动 Gateway
- [ ] 新编译的版本生效
- [ ] 单元测试通过
...
**验收标准**:
- [ ] SystemPromptBuilder 在 Engine/Agent 中实际使用
- [ ] 完整 system prompt 包含 agents.md 内容
- [ ] 完整 system prompt 包含 skills 描述
- [ ] 完整 system prompt 包含工具列表
- [ ] LLM 调用时正确注入 system prompt
- [ ] 单元测试通过
...
**架构约束**:
- Gateway 持有 Engine 和 PluginManager 实例
- Gateway 生命周期与 daemon 进程绑定
- RPC server 使用 Unix Socket（本地安全）
...
**架构约束**:
- 复用 pkg/protocol 已有类型定义
- Unix Socket 权限：仅当前用户可访问 (0600)
...
- [x] 基础样式通过 lipgloss 支持（完整语法高亮留待后续）
...
**实现步骤**:
1. 实现 `pc task list`：
   - 调用 Gateway RPC `ListTasks`
   - 格式化输出表格
   - 支持 `--status` 过滤
2. 实现 `pc task add <title>`：
   - 调用 Gateway RPC `AddTask`
   - 支持 `--description` 参数
3. 实现 `pc task complete <id>`：
   - 调用 Gateway RPC `CompleteTask`
4. 实现 `pc task delete <id>`：
   - 调用 Gateway RPC `DeleteTask`
5. 统一输出格式（表格或 JSON）
6. 编写表格驱动测试
...
└─────────┼─────────────────┼─────────────────┼──────────┘
          ...
          ▼                 ▼                 ▼
┌─────────────────────────────────────────────────────────┐
│                   RPC Client (pkg/protocol)              │
│              Unix Socket: ~/.pc/pc.sock                 │
└─────────────────────────────────────────────────────────┘
          │
          ▼
┌─────────────────────────────────────────────────────────┐
...
| internal/skill/executor.go:37 | Add plugin manager | ✅ 已完成 | - |
| internal/skill/executor.go:146 | tool calling via plugin manager | ✅ 已完成 | - |
| examples/plugins/... | 示例插件完善 | ✅ 已完成 | - |
| cmd/pc/... | CLI 框架和子命令 | ✅ 已完成 | M7-001 |
| internal/gateway/... | Gateway daemon 核心 | ✅ 已完成 | M7-002 |
| internal/gateway/rpc.go | RPC 协议实现 | ✅ 已完成 | M7-003 |
| internal/tui/... | TUI 交互界面 | ✅ 已完成 | M7-004 |
| internal/task/client.go | Task CLI client | ✅ 已完成 | M7-005 |
| cmd/pc/gateway/restart.go | Gateway restart 命令 | 🆕 待实现 | M9-001 |
| internal/agent/system_prompt.go | SystemPromptBuilder 集成 | 🆕 待实现 | M9-002 |
| internal/config/config.go | agents.md 路径配置 | 🆕 待实现 | M9-003 |
...
| 0.6 | 2026-02-27 | 完成所有技术债务（REF-001~004），完成 M6 所有任务，skill executor 集成 plugin manager |
| 0.7 | 2026-02-27 | 重构 M7 里程碑，添加架构映射和详细实现步骤（M7-001~M7-006），补充 CLI 层架构图 |
| 0.8 | 2026-02-27 | 新增 M7-005 (TUI 富功能)、M7-006 (TUI 与 Channel 共存)，原 M7-005/006 顺延为 M7-007/008 |
| 0.9 | 2026-02-28 | 新增 M9 里程碑 (M9-001~003)，整理 05.todo.md 待定需求到 spec/arch/task |
