...
func (b *SystemPromptBuilder) buildToolExamples() string {
	...
	if hasShellTool {
		examples = append(examples, "**示例 1: 执行 bash 命令**")
		examples = append(examples, "")
		examples = append(examples, "用户: 列出主目录的文件")
		examples = append(examples, "")
		examples = append(examples, "你的回复:")
		examples = append(examples, "<thinking>")
		examples = append(examples, "用户想要查看主目录的文件，我可以使用 bash 工具执行 ls 命令。")
		examples = append(examples, "</thinking>")
		examples = append(examples, "")
		examples = append(examples, "comment:tool_call")
		examples = append(examples, "<name>bash</name>")
