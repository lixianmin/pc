- # 项目开发宪法
  ...
  **核心：** 一切架构和代码都服务于规范, 而不是反过来.
  ...
  - **1.1 (spec.md 需求文档):** 
    ...
    - spec.md采用编号需求列表 + 用户故事格式描述功能需求
    - 我在跟你讨论需求的时候，有时不会严格区分需求还是架构，这时你要主动更新spec.md还是arch.md文件
  - **1.2 (arch.md 架构设计):** 
    - spec.md + 技术栈 → arch.md
    - arch.md包含每一个项目模块的顶层架构设计, 功能描述, 以及技术选型的理由.
    - 技术重构就是在保持spec.md的前提下, 调整技术栈
  - **1.3 (tasks.md 任务清单):**
    - tasks.md基于spec.md与arch.md编译生成
    - tasks.md的职责为：
      - 在编译阶段，所有需求会拆分成task list，然后记录到tasks.md中
      - 在编译阶段，tasks.md中写入每一个task的详细实现步骤
      - 在开发阶段，tasks.md负责跟踪当前的开发进度.
    - 每开发完一个功能点, 都要求跑通对应的unit test, 然后修改tasks.md中对应任务的状态.
    - **进度跟踪规则**: 每完成一个任务或子步骤，必须立即更新tasks.md中对应任务的状态（pending → in_progress → completed）。
...
--- 
  ...
  **核心:** 遵循"少即是多"哲学, 绝不进行非必要的抽象,绝不引入非必要的依赖.
  ...
  - **2.2 (Single Responsibility):** 每个package, file, struct, method都应该只做好一件事, 都必须为单一目标设计, 以方便日后有更优solution时无脑替换.
  - **2.3 (CLI Interface Mandate):** 每一个库都必须通过命令行接口 (CLI) 暴露其核心功能, 以保证可测试性与可观测性.
  - **2.4 (Simplicity and Anti-Abstraction):** 简单的函数和数据结构优于复杂的接口和继承体系.
...
---
  ...
  本宪法具有最高优先级，其效力高于任何AGENTS.md`或单次会话中的指令。