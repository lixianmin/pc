...
- [x] 新消息到达时自动滚动到底部（如果用户未手动滚动）
- [x] 用户手动滚动后保持当前位置
- [x] 用户输入新消息时恢复自动滚动
- [x] 滚动到底部时重置 userScrolled 状态
- [ ] 单元测试验证滚动行为
...
**优先级**: P1 | **需求**: CLI-016
**架构映射**: `internal/tui/model.go` (View 方法)

### 实现步骤

1. ~~创建 `view.go` 文件~~ (实现保留在 model.go)
2. 在 `View()` 方法中集成 scrollbar 渲染
3. 使用 lipgloss 进行布局
4. 集成 scrollbar 到主布局
5. 保持现有样式
...
- [x] Scrollbar 正确集成到布局 (在 model.go 中)
- [x] 界面渲染正常
- [x] 窗口大小变化时正确调整
- [x] 代码结构清晰
