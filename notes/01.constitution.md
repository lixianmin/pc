- # 项目开发宪法
  ...
  **核心：** 一切架构和代码都服务于规范, 而不是反过来.
  ...
  - **1.2 (arch.md 架构设计):** 
    - 文件地址：./notes/03.arch.md
    - spec.md + 技术栈 → arch.md
    - arch.md包含每一个项目模块的顶层架构设计, 功能描述, 以及技术选型的理由.
    - 技术重构就是在保持spec.md的前提下, 调整技术栈
  - **1.3 (task.md 任务清单):**
    - 文件地址：./notes/04.task.md
    - task.md基于spec.md与arch.md编译生成
    - task.md的职责为：
      - 在编译阶段，所有需求会拆分成task list，然后记录到task.md中
      - 在编译阶段，task.md中写入每一个task的详细实现步骤
      - 在开发阶段，task.md负责跟踪当前的开发进度.
    - 每开发完一个功能点, 都要求跑通对应的unit test, 然后修改task.md中对应任务的状态.
    - **进度跟踪规则**: 每完成一个任务或子步骤，必须立即更新task.md中对应任务的状态（pending → in_progress → completed）。
...
---
  ...
  **核心：** 遵循“少即是多”哲学, 绝不进行非必要的抽象，绝不引入非必要的依赖.
  ...
  - **3.2 (Single Responsibility):** 每个package, file, struct, method都应该只做好一件事, 都必须为单一目标设计, 以方便日后有更优solution时无脑替换.
